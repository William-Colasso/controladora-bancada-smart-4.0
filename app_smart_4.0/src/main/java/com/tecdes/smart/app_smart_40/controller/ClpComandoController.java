 ...
 *
 * <p>Cada chamada executa <b>uma</b> passada de leitura+escrita; o caller (frontend, via poller) repete.
 * O reflexo visual do novo estado chega ao frontend pelo SSE ({@code estacao-status}/{@code estoque}/
 * {@code expedicao}) — esta API não devolve o estado derivado, só confirma a passada.
 *
 * <p>Rotas:
 * <ul>
 *   <li>{@code POST /api/clp/{estacao}/processar} — uma passada na estação (estoque/processo/montagem/expedicao).</li>

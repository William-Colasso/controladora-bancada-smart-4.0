...
**Write/command path — on-demand REST (`ClpComandoService` + `ClpComandoController`).** The write handshake (`RecebidoOP`, magazine add/remove) lives in the `*ClpService` (`lerEProcessar`/`processData`), which now implement `EstacaoClpHandshake` (`estacao()` + `lerEProcessar(ip)`). `ClpComandoService` injects `List<EstacaoClpHandshake>` into an `EnumMap` and dispatches per station, resolving the IP from `ClpIpRegistry` **on each call** (same source as the read side). `ClpComandoController` exposes `POST /api/clp/{estacao}/processar` (one pass on a station) and `POST /api/clp/processar` (all four). It **also hosts the read opt-in endpoints** `POST /api/clp/{estacao}/conectar` and `/desconectar` (see the Read/leitura opt-in path below) — there is no separate `ClpConexaoController`. **No scheduler: each call is a single pass — the caller repeats** (frontend drives it with `createPoller`). The resulting state reaches the browser via the **SSE** channel, not the POST response (which only confirms the pass). Each `lerEProcessar` wraps `readBlock`+`processData` in `synchronized(connector)` to serialize with the read-only SSE reads on the same S7 socket; the writes inside `processData` stay gated by `estado.isReadOnly()`.

**Read/leitura opt-in path — per-station (`ClpLeituraRegistry` + `ClpComandoController` + `ClpHealthService`).** The SSE status producers do **not** read at boot — reading is opt-in per station. `ClpLeituraRegistry` (`@Component`, `synchronized`, `EnumMap`) holds an enabled flag per `EstacaoClp`, **all false by default**. `ClpComandoController` (`POST /api/clp/{estacao}/conectar` body `{"ip":"x.x.x.x"}` — same controller as the `/processar` write path) does: `ipRegistry.setIp` → `ClpHealthService.alcancavel(ip)` (a **TCP connect to port 102**, the real S7 link — not ICMP, ~1.5s timeout) → only on success `leituraRegistry.habilitar(estacao)` and returns `ClpConexaoResponseDTO(estacao, ip, alcancavel, leitura)`; unreachable → 200 with `alcancavel:false, leitura:false` (reading stays off). `POST /api/clp/{estacao}/desconectar` disables it. Each status producer checks `leituraRegistry.isHabilitada(estacao)` at the top of `poll()` and returns early (clearing its `ultimo` cache so re-enabling re-emits) when disabled — so a station only hits the S7 socket and emits `estacao-status` after the user connects it.
...
- **Routing/delivery** (`service/sse/`): `SseNotifier` (one `@Async @EventListener` per event type → `registry.broadcast(name, dto)`); `SseEmitterRegistry` (`CopyOnWriteArrayList<SseEmitter>`, removes dead clients per-send so one failure doesn't break the rest). Because producers publish **only on change**, the registry caches the **last payload per event name** (`broadcast` stores it) and **replays the snapshot to each newly-connected client** (`add`) — otherwise a browser opening after the state settled would see nothing. `SseController` `GET /api/stream` (`text/event-stream`). SSE event names: `estacao-status`, `estoque`, `expedicao`.
- **Adding a new source** = new producer + new event record + one `@EventListener` in `SseNotifier`. Nothing existing is touched.
- **Frontend:** `core/sse.js` (`createSse(path)` → `EventSource` wrapper with `on(evento, cb)` + auto-reconnect backoff). `pages/dashboard.js` consumes `estoque`/`expedicao`; `pages/home.js` feeds `estacao-status` into `bancadaStatus.setEstado/setFuncionamento`; `pages/estacoes.js` (the `/estacoes` page) is a pure `estacao-status` consumer that paints a live card per station (no DB, no `/api/**`) — the card `id` is the SSE `frontKey`. `home.js`'s per-station **Conectar** button is a **toggle** that drives the opt-in read path: Conectar → `POST /api/clp/{estacao}/conectar` (green "Lendo" badge if `alcancavel && leitura`, red otherwise); Desconectar → `POST /api/clp/{estacao}/desconectar` and clears that station's overlay. The row id (`estoque-clp-ip` → `estoque`) is the REST `apiName`; `home.js` maps it to the SSE/overlay `frontKey` (only `processo`→`producao`).

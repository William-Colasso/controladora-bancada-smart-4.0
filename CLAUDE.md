...
**Write/command path — on-demand REST (`ClpComandoService` + `ClpComandoController`).** The write handshake (`RecebidoOP`, magazine add/remove) lives in the `*ClpService` (`lerEProcessar`/`processData`), which now implement `EstacaoClpHandshake` (`estacao()` + `lerEProcessar(ip)`). `ClpComandoService` injects `List<EstacaoClpHandshake>` into an `EnumMap` and dispatches per station, resolving the IP from `ClpIpRegistry` **on each call** (same source as the read side). `ClpComandoController` exposes only `POST /api/clp/{estacao}/processar` (one pass on a station) and `POST /api/clp/processar` (all four). After each pass `ClpComandoService.processar()` **publishes an `EstacaoAllData` event** (the full `*CLP` bean for that station) onto the bus → broadcast as SSE `estacao-all` (so the write path also feeds the browser, beyond the read-only producers). The snapshot is exposed through `EstacaoClpHandshake.dados()` (each `*ClpService` returns its `*CLP` bean), keeping `ClpComandoService` agnostic of the concrete beans. **Driven automatically by `ClpProcessamentoScheduler`** (`@Scheduled(fixedDelayString="${clp.processar.interval:300}")`, default **300ms**) which calls `processarTodas()` — **gated by SSE client presence** (`sseRegistry.count() == 0` → skip, same model as the read-only producers): opening any SSE page runs the handshake for **all** stations, closing the last tab stops it. There is **no** front-end toggle/poller and no manual "start" step. (`ClpComandoController` still exposes `POST /api/clp/{estacao}/processar` and `POST /api/clp/processar` for a single manual pass, but they're no longer the normal driver.) The resulting state reaches the browser via the **SSE** channel, not the POST response. Each `lerEProcessar` wraps `readBlock`+`processData` in `synchronized(connector)` to serialize with the read-only SSE reads on the same S7 socket; the writes inside `processData` stay gated by `estado.isReadOnly()`. **The `*CLP` beans are filled ONLY here** (top of `processData`), which also stamps `EstadoProducaoService.ultimoLeituraMillis = now` — the **freshness** signal that gates the periodic `estacao-all` producers (below). With no SSE client connected (or IP unset / CLP unreachable), nothing fills the beans, so `estacao-all` stops (rather than re-broadcasting stale all-`false/null/0` data).

**Read/leitura gating — by SSE client presence (`SseEmitterRegistry.count()`).** The SSE producers do **not** read at boot, and there is **no per-station opt-in** (the old `ClpLeituraRegistry` + `/conectar`/`/desconectar` endpoints + `ClpHealthService` were removed). Instead, every producer checks `sseRegistry.count() == 0` at the top of `poll()` and returns early (clearing its `ultimo` cache so a reconnect re-emits) — so the S7 socket / MySQL is only hit while **at least one browser has an open `EventSource`** on `GET /api/stream`. Opening *any* SSE-consuming page (home, `/estacoes`, dashboard) is enough to start reading **all** stations; closing the last tab stops it. This makes the live data work on every screen without a manual "connect" step, while still idling when nobody is watching. IPs are configured **separately** via `ClpIpController` (`PUT /api/clp/ips/{estacao}`) — that is the only job of the home "connection" UI now.
...
- **Producers** (`service/sse/producer/`, `@Scheduled(fixedDelayString=...)`, publish **only on change** vs a cached last snapshot):
  ...
  - **`estacao-all` re-broadcasters** (`service/sse/producer/clp_data/`): abstract `EstacaoCLPProducer` + 4 `@Component` subclasses (`Estoque/Expedicao/Processo/MontagemCLPProducer`), each with its own `@Scheduled` cadence — **estoque/expedicao 300ms, processo/montagem 1s** (`${clp.poll.<estacao>:...}`). These do **not** read the PLC — they just re-publish the in-memory `*CLP` bean (filled by the write path) as `EstacaoAllData`. **Gated by freshness:** `publicar()` skips if `now - EstadoProducaoService.ultimoLeituraMillis > 1500ms`, so they go silent when the comunicação driver is stopped (no stale all-`false/null/0` spam). The bean is injected by concrete type (`@Component("estoque")` etc.), the same singleton the matching `*ClpService` fills.
- **Events** (`dto/event/`, records): `EstacaoStatusEvent(estacao, estado, funcionamento)`, `EstoqueGridEvent(posicoes)`, `ExpedicaoGridEvent(posicoes)`, `EstacaoAllData(estacao, dados)` (full `*CLP` bean; published by the **write path** `ClpComandoService` after each pass **and** by the periodic `clp_data` re-broadcasters — both gated by reading freshness, so neither emits once the comunicação driver stops).
- **Routing/delivery** (`service/sse/`): `SseNotifier` (one `@Async @EventListener` per event type → `registry.broadcast(name, dto)`); `SseEmitterRegistry` (`CopyOnWriteArrayList<SseEmitter>`, removes dead clients per-send so one failure doesn't break the rest). Because producers publish **only on change**, the registry caches the **last payload per (event name + key)** (`broadcast` stores it) and **replays each cached snapshot to every newly-connected client** (`add`) — otherwise a browser opening after the state settled would see nothing. `SseController` `GET /api/stream` (`text/event-stream`). SSE event names: `estacao-status`, `estoque`, `expedicao`, `estacao-all`. **`estacao-status` and `estacao-all` are generic across the 4 stations**, so `broadcast` has an overload `broadcast(evento, chave, dado)` — `SseNotifier` passes `evento.estacao()` as the key so the cache holds **one entry per station** (`estacao-all::estoque`, …) and replay-on-connect delivers **all four** (the single-arg `broadcast(evento, dado)` keys by event name for the singleton `estoque`/`expedicao` grids). The SSE event name sent to the browser is unchanged — only the cache key differs.
- **Adding a new source** = new producer + new event record + one `@EventListener` in `SseNotifier`. Nothing existing is touched.
- **Frontend:** `core/sse.js` (`createSse(path)` → `EventSource` wrapper with `on(evento, cb)` + auto-reconnect backoff). `pages/dashboard.js` consumes `estoque`/`expedicao`; `pages/home.js` feeds `estacao-status` into `bancadaStatus.setEstado/setFuncionamento`; `pages/estacoes.js` (the `/estacoes` page) consumes **both** `estacao-status` (card badge/operação **and** the `bancada-status` component shown on top via `bancadaStatus.setEstado/setFuncionamento`) and `estacao-all` (the full `*CLP` bean rendered as a generic key/value grid per card — iterates whatever fields the backend sends, no hardcoded schema). No DB, no `/api/**`; the card `id` is the SSE `frontKey`. Every SSE-consuming page just calls `createSse().connect()`; opening it is what triggers reading (client-presence gating above). `home.js`'s per-station button now only **saves the IP** (`PUT /api/clp/ips/{estacao}`, "IP salvo"/"IP inválido" badge) and prefills the inputs from `GET /api/clp/ips` on load — it no longer enables/disables reading. The row id (`estoque-clp-ip` → `estoque`) is the REST `apiName`; the SSE/overlay `frontKey` differs only for `processo`→`producao` (mapping done backend-side in the event payload).
  - **No comunicação toggle:** the write/handshake loop runs **automatically** server-side (`ClpProcessamentoScheduler`, gated by SSE presence) — there is no start/stop button. `home.js` keeps only an **informative** `#comunicacao-status` badge driven by an `estacao-all` heartbeat: each event stamps `ultimaLeitura` and a 1s `setInterval` shows green "Leitura automática ativa" (event in ≤2500ms) or dim "Sem leitura do CLP" (IP unset / CLP unreachable / no client).
  - **`/estacoes` freshness watchdog:** same heartbeat idea — because the `estacao-all` producers are freshness-gated server-side, `estacoes.js` stamps `ultimaLeitura[estacao]` on each `estacao-all` and a 1s `setInterval` flips any card with no event in >2500ms to `data-comunicacao="off"` + "Aguardando comunicação CLP…" — so an idle (all-zero) station that's actually being read is **not** confused with reading that has stopped.

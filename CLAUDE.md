...
- **Events** (`dto/event/`, records): `EstacaoStatusEvent(estacao, estado, funcionamento)`, `EstoqueGridEvent(posicoes)`, `ExpedicaoGridEvent(posicoes)`, `EstacaoAllData(estacao, dados)` (full `*CLP` bean; published by the **write path** `ClpComandoService`, not by a read-only producer).
- **Routing/delivery** (`service/sse/`): `SseNotifier` (one `@Async @EventListener` per event type → `registry.broadcast(name, dto)`); `SseEmitterRegistry` (`CopyOnWriteArrayList<SseEmitter>`, removes dead clients per-send so one failure doesn't break the rest). Because producers publish **only on change**, the registry caches the **last payload per event name** (`broadcast` stores it) and **replays the snapshot to each newly-connected client** (`add`) — otherwise a browser opening after the state settled would see nothing. `SseController` `GET /api/stream` (`text/event-stream`). SSE event names: `estacao-status`, `estoque`, `expedicao`, `estacao-all`. Note: `estacao-all` (like `estacao-status`) is **generic across the 4 stations** — the registry's replay-on-connect cache keys by event name, so it replays only the **last** station emitted; while the write driver runs (~1/s) the four re-emit quickly.
